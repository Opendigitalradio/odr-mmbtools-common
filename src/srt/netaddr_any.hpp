...
written by
   ...
   This is a copy of sockaddr_any.h from SRT, just to have a definite version here.
 *****************************************************************************/
...
struct netaddr_any
{
    ...
    explicit netaddr_any(int domain = AF_UNSPEC) {
    ...
    netaddr_any(const sockaddr_storage& stor) {
        // Here the length isn't passed, so just rely on family.
        set((const sockaddr*)&stor);
    }

    netaddr_any(const sockaddr* source, len_t namelen = 0) {
        if (namelen == 0)
            set(source);
        else
            set(source, namelen);
    ...
    netaddr_any(const in_addr& i4_adr, uint16_t port) {
    ...
    netaddr_any(const in6_addr& i6_adr, uint16_t port) {
    ...
    bool empty() const {
        ...
        if (sa.sa_family == AF_INET) {
            isempty = (sin.sin_port == 0 && sin.sin_addr.s_addr == 0);
        }
    ...
    struct Equal
    {
        bool operator()(const netaddr_any& c1, const netaddr_any& c2)
        {
            ...
            // Therefore netaddr_any class cannot operate with it,
    ...
    bool operator==(const netaddr_any& other) const
    {
        return Equal()(*this, other);
    }

    bool operator!=(const netaddr_any& other) const { return !(*this == other); }
};

template<> struct netaddr_any::TypeMap<AF_INET> { typedef sockaddr_in type; };
template<> struct netaddr_any::TypeMap<AF_INET6> { typedef sockaddr_in6 type; };

template <>
inline netaddr_any::TypeMap<AF_INET>::type& netaddr_any::get<AF_INET>() { return sin; }
template <>
inline netaddr_any::TypeMap<AF_INET6>::type& netaddr_any::get<AF_INET6>() { return sin6; }

netaddr_any create_addr(const std::string& name, unsigned short port, int pref_family = AF_UNSPEC);
