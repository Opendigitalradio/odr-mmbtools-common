...
using shared_srt = shared_ptr<socket>;

socket::socket(std::string host,
        uint16_t port, bool blocking,
        std::map<string, string> options) :
    m_blocking_mode(blocking), m_host(host), m_port(port), m_options(options)
{
    ...
    if (m_options.count("blocking")) {
        throw srt::exception("Don't set blocking in the options");
    }
...
socket::socket(const int sock, bool blocking)
    : m_bind_socket(sock)
...
void socket::raise_exception(const string &&place) const
{
    ...
    throw srt::exception(place + ": " + message);
}

void socket::raise_exception(const string &&place, const string &&reason) const
{
    //spdlog::debug(LOG_SOCK_SRT "@{} {}. ERROR: {}.", m_bind_socket, place, reason);
    throw srt::exception(place + ": " + reason);
}

shared_srt socket::connect()
{
    netaddr_any sa;
    try
    {
...

    {
        ...
        if (res == SRT_ERROR)
            raise_exception("connect::configure_post");
    }
...
void socket::assert_options_valid(const std::map<string, string>& options, const unordered_set<string>& extra)
{
#ifdef ENABLE_CXX17
    ...
    for (const auto& el : options)
    {
        ...
        throw srt::exception(ss.str());
    }
}

void socket::assert_options_valid() const
{
    assert_options_valid(m_options, {"bind", "mode"});
}

int socket::configure_pre(SRTSOCKET sock)
{
...
std::string socket::print_negotiated_config(SRTSOCKET sock)
{
...
#define VAL_AND_STR(X) X, "X"
    ...
    if (get_sock_value(sock, VAL_AND_STR(SRTO_TSBPDMODE)) > 0) {
        ...
        latency_ss << "Latency RCV " << latency_rcv << "ms, peer " << latency_peer << "ms";
    }
    else {
        latency_ss << "off";
    }
...
#undef VAL_AND_STR
    ...
    stringstream ss;
    ss << "TSBPD " << latency_ss.str() << ". KM state " <<
        convert(km_state, km_states) << " (RCV " <<
        convert(km_state_rcv, km_states) << ", SND " <<
        convert(km_state_snd, km_states) << "). PB key length: " <<
        pbkeylen << ". Cryptomode " << crypto_mode_str << ". Stream ID: " <<
        (streamid_len > 0 ? streamid : "not set");
    return ss.str();
}

int socket::configure_post(SRTSOCKET sock) const
{
    ...
    if (!failures.empty())
    {
        stringstream ss;
        copy(failures.begin(), failures.end(), ostream_iterator<string>(ss, ", "));
        cerr << "failed to set options: " << ss.str() << "\n";
    }

    return 0;
}

void socket::handle_hosts()
{
    ...
    if (m_options.count("bind"))
    {
        ...
        std::transform(bindipport.begin(), bindipport.end(), bindipport.begin(), [](char c) { return tolower(c); });
        ...
        {
            throw srt::exception("create_addr_inet failed");
        }
    ...
    else if (m_mode == connection_mode::LISTENER)
    {
        ...
        bind_me(reinterpret_cast<const sockaddr*>(&sa));
        cerr << "SRT Listen " << m_host << ":" << m_port << "\n";
        //spdlog::info(LOG_SOCK_SRT "srt://{0}:{1:d}: bound to '{0}:{1:d}'.",
        //  m_host, m_port);
    }
}

buffer socket::read(size_t max_len, int timeout_ms)
{
    if (!m_blocking_mode)
    {
    ...
    buffer buf(max_len);
    const int res = srt_recvmsg2(m_bind_socket, reinterpret_cast<char*>(buf.data()), (int)buf.size(), nullptr);
    if (SRT_ERROR == res)
    {
...
int socket::write(const buffer &buffer, int timeout_ms)
{
    if (!m_blocking_mode)
    {
        int ready[2] = {SRT_INVALID_SOCK, SRT_INVALID_SOCK};
    ...
    const int res = srt_sendmsg2(m_bind_socket, reinterpret_cast<const char*>(buffer.data()), static_cast<int>(buffer.size()), nullptr);
    if (res == SRT_ERROR) {
...
socket::connection_mode socket::mode() const
{
    return m_mode;
}

int socket::statistics(SRT_TRACEBSTATS& stats, bool instant)
{
    return srt_bstats(m_bind_socket, &stats, instant);
}

const string socket::stats_to_csv(int socketid, const SRT_TRACEBSTATS& stats, bool print_header)
{
...
#if 0
const nlohmann::json socket::stats_to_json(int socketid, const SRT_TRACEBSTATS& stats)
{
    nlohmann::json root;
...
#if 0
const string socket::get_statistics(string stats_format, bool print_header) const
{
